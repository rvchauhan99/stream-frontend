    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-dark-8 p-4 rounded-lg flex items-start justify-between">
          ...
          <ShowChartIcon className="text-red-45 text-3xl" />
        </div>
        ...
        <div className="bg-dark-8 p-4 rounded-lg flex items-start justify-between">
          ...
          <VideoLibraryIcon className="text-red-45 text-3xl" />
        </div>
        ...
        <div className="bg-dark-8 p-4 rounded-lg flex items-start justify-between">
          ...
          <PeopleIcon className="text-red-45 text-3xl" />
        </div>

        <div className="bg-dark-8 p-4 rounded-lg flex items-start justify-between">
          <div>
            <p className="text-grey-70">Total Likes</p>
            <h3 className="text-2xl font-bold text-white mt-2">128K</h3>
            <p className="text-green-500 text-sm">+5K this month</p>
          </div>
          <ThumbUpIcon className="text-red-45 text-3xl" />
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
        ...
        <button className="bg-dark-8 p-6 rounded-lg flex flex-col items-center justify-center hover:bg-dark-10 transition-colors">
          <CloudUploadIcon className="text-red-45 text-4xl mb-2" />
          <span className="text-white">Manage Content</span>
        </button>
        <button className="bg-dark-8 p-6 rounded-lg flex flex-col items-center justify-center hover:bg-dark-10 transition-colors">
          <SettingsIcon className="text-red-45 text-4xl mb-2" />
          <span className="text-white">Channel Settings</span>
        </button>
        <button className="bg-dark-8 p-6 rounded-lg flex flex-col items-center justify-center hover:bg-dark-10 transition-colors">
          <AssessmentIcon className="text-red-45 text-4xl mb-2" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-dark-8 p-4 rounded-lg">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-white font-semibold">Channel Growth</h3>
            <button className="text-red-45 hover:text-red-60">Show All</button>
          </div>
          <BarChart width={500} height={300} data={viewsData} className="mx-auto">
            <CartesianGrid strokeDasharray="3 3" stroke="#333" />
            <XAxis dataKey="year" stroke="#666" />
            <YAxis stroke="#666" />
            <Tooltip contentStyle={{ backgroundColor: '#1A1A1A', border: 'none' }} />
            <Legend />
            <Bar dataKey="views" fill="#E30000" />
            <Bar dataKey="engagement" fill="#FF3333" />
            <Bar dataKey="subscribers" fill="#FF6666" />
          </BarChart>
        </div>
        ...
        <div className="bg-dark-8 p-4 rounded-lg">
          ...
          <AreaChart width={500} height={300} data={revenueData} className="mx-auto">
            <CartesianGrid strokeDasharray="3 3" stroke="#333" />
            <XAxis dataKey="month" stroke="#666" />
            <YAxis stroke="#666" />
            <Tooltip contentStyle={{ backgroundColor: '#1A1A1A', border: 'none' }} />
            <Legend />
            <Area type="monotone" dataKey="adsRevenue" fill="#E30000" stroke="#E30000" />
            <Area type="monotone" dataKey="sponsorships" fill="#FF3333" stroke="#FF3333" />
          </AreaChart>
        </div>
      </div>
          ...
          <table className="w-full">
            ...
            <tbody>
              {latestComments.map((comment, index) => (
                <tr key={index} className="border-b border-dark-15">
                  <td className="py-3 px-4 text-grey-70">{comment.date}</td>
                  <td className="py-3 px-4 text-grey-70">{comment.user}</td>
                  <td className="py-3 px-4 text-grey-70">{comment.video}</td>
                  <td className="py-3 px-4 text-grey-70">{comment.comment}</td>
                  <td className="py-3 px-4">
                    <span className={`px-2 py-1 rounded text-xs ${
